...
def train(hparams, num_workers, node_id, fold, kfold, debug, path):
    CFG = AttrDict(
        batch_size       = 80,
    ...
    stopping_cb = tf.keras.callbacks.EarlyStopping(monitor='val_auc', min_delta=1e-5,
                                                   mode='max', patience=1+CFG.epochs//2, verbose=1,
...
#     tqdm_progbar = tfa.callbacks.TQDMProgressBar()
    history_csv = OUTPUTPATH + '/history.csv'
    csv_cb = tf.keras.callbacks.CSVLogger(history_csv, append=True)

    callbacks = [checkpoint_cb, stopping_cb, lr_sched_cb, tensorboard_cb, csv_cb]
#     else:
#         callbacks = [checkpoint_cb, stopping_cb, lr_sched_cb]



    # In[20]:

    print(f'Starting training {node_id}:', callbacks)
    model.fit(ds_train,
              verbose          = int(node_id == 0),
    ...
    print(history)
    return history['val_auc'].max()



if __name__ == "__main__":
    path = os.environ['SCRATCH'] + f"/summer_school/melanoma_f{fold}/"
    ...
    print(hparams)
    res = train(hparams, num_workers, node_id, fold, kfold, debug, path)
    print(res)
